...
function findShortArg(expected: CLIOptionList, arg: string): CLIOption | null {
  for (let i = 0; i < expected.length; i++) {
    ...
    if (element.short === arg) {
      ...
      arg.startsWith(element.short) &&
      element.params === 1 &&
      element.allowPrefixed
    ) {
 ...
 */
export function handleCLIOptions(
  expected: CLIOptionList,
  args: string[] | null,
  ...
  while (i < argv.length) {
    ...
    if (argument === null) {
      ...
      if (
        (argument.params >= 0 && argument.params !== list.length) ||
        (argument.params < 0 && list.length <= 0)
      ) {
